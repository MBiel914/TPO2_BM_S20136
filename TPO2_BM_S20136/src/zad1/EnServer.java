...
public class EnServer {
	private static EnServer mainClient;
    private static Socket mainSocket;
    private static Map<String, String> dictionary;
    
	public static void main(String[] args) throws IOException, InterruptedException {
        mainClient = new EnServer("127.0.0.1", 10666);
        LoadDictionary();
        
        try {
			System.out.println("English server port: " +  mainSocket.getLocalPort());
		} catch (Exception e) {
			e.printStackTrace();
		}
    ...
    private static void LoadDictionary() {
    	...
    	dictionary.putIfAbsent("dom", "house");
    	dictionary.putIfAbsent("kot", "cat");
    	dictionary.putIfAbsent("pies", "dog");
	}
	
    public EnServer(String adress, int port) throws IOException {
    ...
    void say() throws IOException, InterruptedException {
        ...
        mainPrintWriter.println("Init,EN," + mainSocket.getLocalPort());
    }
                            		...
                            		if(dataMessage[0].equals("Trans"))
                            		{
                            			try (Socket socket = new Socket("127.0.0.1", Integer.parseInt(dataMessage[2]))) {
											...
											if (!dictionary.containsKey(dataMessage[1]))
												serverPrintWriter.println("Error - no \"" + dataMessage[1] + "\" translation on EN server.");
